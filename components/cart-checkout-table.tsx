  ...
  return (
    <div className="h-full ">
      {cart.length > 0 ? (
                ...
                <TableRow key={product.id}>
                  ...
                  <TableCell className="">
                    <div className="flex items-center border border-black rounded-md w-min">
                      <Button onClick={()=> decreaseAmount(product.id)} className="text-black bg-transparent">
                        <Minus className="w-5 aspect-square" />
                      </Button>
                      <p>{product.amount}</p>
                      <Button onClick={()=> increaseAmount(product.id)} className="text-black bg-transparent">
        ...
        <p className="flex items-center justify-center h-full font-semibold text-center">Your cart is empty</p>
      )}
    </div>
  );
}
